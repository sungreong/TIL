...
class TabularNN(Utils, LayerUtils):
    ...
    def Projector(self, train_writer, cat_col, LabelEncoding):
        ...
        for cat in cat_col:
            ...
            metadata_file = os.path.join(ResultPath, f'train',
                                         f'metadata_{cat}.tsv')
    ...
    def train(self, LabelEncoding, Train, Valid, mb_size, Epoch, config):
        ...
        train_writer = tf.summary.FileWriter(os.path.join(ResultPath, f'train'), sess.graph)
        saver = tf.train.Saver()
        self.Projector(train_writer, self.cat_col, LabelEncoding)
        ...
        for epoch in range(Epoch):
            ...
            if epoch % 100 == 0:
                ...
                saver.save(sess,
                           os.path.join(ResultPath, f'train', "model.ckpt"),
        ...
        global best_auc
        if best_auc < AUC:
            self.log.info("{} < {}".format(best_auc, AUC))
            best_auc = AUC
        clear_output()
        tune.track.log(auc=AUC, done=True)
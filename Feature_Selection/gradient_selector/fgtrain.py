...
def get_optim_f_stop(maxiter, maxtime, dftol_stop, freltol_stop, minibatch=True):
    ...
    def f_stop(f0, v0, it, t):
        ...
        if ((not minibatch) and (df < dftol_stop)) or (
            minibatch and (df_ma() < dftol_stop)
        ):
            flag_stop = True
...
def get_checkpoint(S, stop_conds, rng=None, get_state=True):
    ...
    if get_state:
        m.update(
            {
                constants.Checkpoint.MODEL: S.state_dict(),
...
def _train(
    ...
    f_stop, stop_conds = get_optim_f_stop(
        maxiter, maxtime, dftol_stop, freltol_stop, minibatch=minibatch
    )

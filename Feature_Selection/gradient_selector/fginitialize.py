...
class PrepareData(Dataset):
    def __init__(
        ...
        assert (
            data_format in constants.DataFormat.ALL_FORMATS
        ), "Format must in {0}.".format(", ".join(constants.DataFormat.ALL_FORMATS))
        ...
        if D is None:
            assert (
                self.disk_size / BYTESPERGB <= self.MAXMEMGB
        ...
        if (
            self.storage_level == constants.StorageLevel.DISK
            and self.format == constants.DataFormat.SVM
            and self.set_params
        ):
    ...
    def compute_data_stats(self):
        ...
        sv1 = scipy.sparse.linalg.svds(
            Xc
            / (
                torch.sqrt(
                    torch.prod(
                        torch.as_tensor(y.size(), dtype=torch.get_default_dtype())
                    )
                )
                if not scipy.sparse.issparse(X)
                else y.numpy().size
    ...
    def apply(self, ix_rows=None, ix_cols=None, f_Xy=None):
        ...
        f_Xy(
            (
                self.X[ix_rows, ix_cols]
                if not self.storage_level == constants.StorageLevel.SPARSE
                else self.X[ix_rows, ix_cols].toarray()
            ),
            self.y[ix_rows],
        )

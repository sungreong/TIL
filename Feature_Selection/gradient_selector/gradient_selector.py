...
class FeatureGradientSelector(FeatureSelector, BaseEstimator, SelectorMixin):
    def __init__(
        ...
        """
        assert (
            order <= 12 and order >= 1
        ), "order must be an integer between 1 and 12, inclusive"
        assert (
            n_features is None or max_features is None
    ...
    def _partial_fit(self, X, y, n_classes=None, groups=None):
        ...
        m, solver = _train(
            ...
            prev_checkpoint=self._prev_checkpoint
            if hasattr(self, "_prev_checkpoint")
            else None,
    ...
    def _set_batch_size(self, data_train):
        ...
        target_batch_size = min(
            max(self.batch_size, self.target_batch_size), data_train.N
        )
    ...
    def transform(self, X):
        ...
        return (
            X.iloc[:, self.selected_features_]
    ...
    def _generate_scores(self, solver, xsub, ysub, step_size, feature_order):
        ...
        for i in np.arange(1, self.max_features + 1, step_size):
            ...
            score = solver.f_train(
                torch.tensor(np.ones(i), dtype=torch.get_default_dtype())
                .unsqueeze(1)
                .to(self.device),
    ...
    def _set_top_features(self, groups=None):
        ...
        self._get_support_mask()
        assert (
            self.n_features <= self.scores_.shape[0]
        ), "n_features must be less than or equal to the number of columns in X"
        # pylint: disable=E1130
        self.selected_features_ = np.argpartition(self.scores_, -self.n_features)[
            -self.n_features :
        ]
    ...
    def _distance_to_line(start_point, end_point, new_point):
        ...
        return np.cross(
            new_point - start_point, end_point - start_point
